...
def plot_results(IXT_train, IXT_validation, ITY_train, ITY_validation,
    loss_train, loss_validation, t, y, epochs, HY, K, fig, ax, problem_type):
    '''
    ...
    ax[1].clear()
    ax[1].plot(lim, np.ones(1000)*HY, color = 'black', linestyle = '--')
    ...
    ax[2].fill_between(diag, 0, np.where(diag>HY, HY, diag), alpha = 0.5, color='darkorange')
    ax[2].plot(diag, np.where(diag>HY, HY, diag), alpha = 0.5, color='blue', linewidth=4)
    ...
    ax[2].annotate(r' $I(X;Y) = H(Y)$', xy=(maxval*0.75,HY*(1.05)), color='darkorange')
    ax[2].annotate(r' $I(X;T) \geq I(T;Y)$', xy=(HY*(0.5),HY*(1.05)), color='darkorange')
...
def plot_behavior(logs_dir,figs_dir,K,betas,hfunc_name,hfunc_param,HY):
